...
function __disconnect() {
    try {
        ...
        // SOCKET FUNKTIONEN SIND NOCH NICHT KORREKT !!!!
...
function __parse(answer) {
    ...
    allKeysAndValues = content.split(';');
    ...
    for (item in allKeysAndValues) {
        ...
        singleKeyAndValueArray = allKeysAndValues[item].split('=');
...
function normalize(keys, values) {  
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (key == 'KDY') {
            tmp_value = (parseFloat(parseInt(value, 16)));
    ...
    // Testweise hier ausgeben
    for (let i = 0; i < keys.length; i++) {
        console.log('Wert von ' + keys[i] + ' ist ' + values[i]);

        //adapter = new utils.Adapter(options);
        adapter.setStateChanged(keys[i], values[i], true);
    }
}
...
function __build_query(values) {
    ...
    l = querystring.length + 2 + 2 + 2 + 2 + 2 + 4;
